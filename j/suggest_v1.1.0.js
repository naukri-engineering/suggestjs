...
/*Start of Suggestor.js (newJquerySugg_7.6.3)*/
(function($) {
    var openDrops = {};
    var sugId;
    var sugInpNode;
    ...
    var setTrackingObject = function(id, textVal, position, typedValue, dataType, cached, Qry) {
        trackingObject[id]['Events'].push(createTrackObject(textVal, position, typedValue, dataType, cached, Qry))
    };
    ...
    $(window).unload(function() {
        ...
        $('.sugInp').each(function(key, val) {
            ...
            var inpVal = $(this).val();
            _params = _params ? JSON.parse(_params) : null;
            if (_params) {
    ...
    $.fn.suggestor = function(options) {
        if (!options.domain) {
            throw Error("Domain parameter is missing, please refer documentation. https://github.com/saeed3e/suggestjs/blob/gh-pages/README.md");
            return false;
        }
        var defaults = {
            url: {
        ...
        var setValueInTextBox = function(btn, relatedConceptFlag) {
            ...
            if (custVal.length < params.maxlength) {
                ...
                if (params.trackUserInteraction) {
                    ...
                    if (params.getTrackUserInteractionData) params.getTrackUserInteractionData(trackingObject);
                }
        ...
        var encodeURL = function(url, queryText, relConcptFlag) {
            ...
            if (_t.Qry) {
                ...
                return url + "query=" + encodeURIComponent(_t.Qry) + appId + '&category=' + categoryList.call(_t, relConcptFlag) + '&limit=' + params.maxSuggestions + sourceId + version;
            } else {
                return false;
            }
        };
        ...
        var checkDatainLocalStorage = function(queryText, type, category) {
            ...
            if (queryText && checkLS()) {
                getData = getLS.call(this, queryText.toLowerCase(), type);
                if (getData) {
        ...
        var assignment = function(node) {
            ...
            //Bind foucs handler on suggestor input node
